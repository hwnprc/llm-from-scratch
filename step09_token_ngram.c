...
/*
 * Step 09: 토큰 단위 N-gram 문장 생성
 *
 * 토큰: 형태소/품사 전체 (예: "나/NP", "는/JX", "고향/NNG")
 * - 공백으로 구분된 tokenized_sentences.txt 사용
 * - 품사 정보 포함하여 더 정확한 문법
 */
...
void train_token_ngrams(const char *filename, int max_n) {
    ...
    printf("토큰 단위 N-gram 학습 중 (2-gram ~ %d-gram)...\n", max_n);
...
int main() {
    ...
    printf("============================================================\n");
    printf("Step 09: 토큰 단위 N-gram 문장 생성 (2-10 gram)\n");
    printf("============================================================\n\n");

    printf("📚 학습 데이터: tokenized_tokens.txt (<W> 구분자, <S> 공백)\n");
    printf("🎯 토큰: 형태소/품사 + 공백 정보\n");
    printf("🔬 실험: 2-gram부터 10-gram까지 과적합 분석\n\n");

    int max_n = 10;
    train_token_ngrams("tokenized_tokens.txt", max_n);

    // 결과 파일
    FILE *fp = fopen("step09_token_ngram_sentences.txt", "w");
    if (!fp) {
        printf("파일 생성 실패\n");
        return 1;
    }
    ...
    fprintf(fp, "============================================================\n");
    fprintf(fp, "토큰 단위 N-gram 문장 생성 결과 (2-10 gram)\n");
    fprintf(fp, "형태소/품사를 토큰으로 사용 + 공백 토큰 <S>\n");
    fprintf(fp, "과적합 분석: N이 커질수록 원문 암기 증가\n");
    fprintf(fp, "============================================================\n\n");
    ...
    printf("✅ 완료!\n");
    printf("============================================================\n");
    printf("결과 파일: step09_token_ngram_sentences.txt\n\n");

    printf("💡 3가지 방식 비교:\n");
    printf("  - step07: 글자 단위 (character-level)\n");
    printf("  - step08: 단어 단위 (word-level, 형태소만)\n");
    printf("  - step09: 토큰 단위 (token-level, 형태소/품사)\n\n");
